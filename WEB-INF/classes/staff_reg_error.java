...
public class staff_reg_error extends HttpServlet {
  
  protected void processRequest(HttpServletRequest request,HttpServletResponse response)
      throws ServletException, IOException {
  request.setCharacterEncoding("Shift_JIS");
  String name = (String)request.getParameter("name");                 /*
  String mailaddress = (String)request.getParameter("mailaddress");      フォームから値を取得
  String password = (String)request.getParameter("password");                                 */
 
  response.setCharacterEncoding("UTF-8");
  HttpSession session = request.getSession();
  session.setAttribute("Password",password);        /*
  session.setAttribute("Name",name);                   セッションに値を設定
  ...
  if (name.length() >= 2 && name.length() <= 20 &&
    ...
    RequestDispatcher dispatcher = context.getRequestDispatcher("/staff_reg_complete.jsp"); //完了画面へ遷移
